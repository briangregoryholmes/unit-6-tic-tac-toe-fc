...
var copySuspensePromise = (suspensePromise) => createSuspensePromise(suspensePromise[SUSPENSE_PROMISE].b, suspensePromise[SUSPENSE_PROMISE].o);
...
var createStore = (initialValues) => {
    ...
    for (const [atom3, value] of initialValues) {
      ...
      if ((import_meta.env && import_meta.env.MODE) !== "production") {
        ...
        if (!hasInitialValue(atom3)) {
          console.warn("Found initial value for derived atom which can cause unexpected behavior", atom3);
        }
      }
  ...
  const setAtomPromiseOrValue = (version, atom3, promiseOrValue, dependencies) => {
    if (promiseOrValue instanceof Promise) {
      ...
      return setAtomSuspensePromise(version, atom3, suspensePromise, dependencies);
    }
    return setAtomValue(version, atom3, promiseOrValue, dependencies);
  };
...
function useAtomValue(atom3, scope) {
  ...
  const [[version, valueFromReducer, atomFromReducer], rerenderIfChanged] = (0, import_react.useReducer)((prev, nextVersion) => {
  ...
  (0, import_react.useEffect)(() => {
    ...
    const unsubscribe = store[SUBSCRIBE_ATOM](atom3, rerenderIfChanged, versionFromProvider2);
...
function useSetAtom(atom3, scope) {
  ...
  const setAtom = (0, import_react.useCallback)((update) => {
    if ((import_meta.env && import_meta.env.MODE) !== "production" && !("write" in atom3)) {
